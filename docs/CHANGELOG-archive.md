...
Entries retired from `docs/CHANGELOG.md` once their phase/increment completed, so the live
changelog stays scannable at session start. Nothing here is superseded by being archived — it is
the same durable project memory, just no longer part of the orientation read.

Numbers are globally unique across this file and `docs/CHANGELOG.md` — never reused, never
reordered. The live changelog carries a milestone summary per archived block; follow an entry
number cited there (or from a later entry) into this file for the full record.

Archived so far: entries 1–221 — the edge tier, the frame store and offline gate validation, the
first learning loop, the two front doors, the edge actuators, and the admin-next rebuild through
its flip.
