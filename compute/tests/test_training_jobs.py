...
class _SuccessGalleryBuilder:
    ...
    def __call__(self, store, out_dir, qualities=None, max_per_cat=None, progress=None):
        self.calls += 1
...
def test_gallery_build_not_enough_writes_no_row_and_surfaces_message(tmp_path):
    ...
    def not_enough(store, out_dir, qualities=None, max_per_cat=None, progress=None):
        return {
            "enough": False,
            "n_crops": 1,
            "n_cats": 1,
...
def test_dedup_guards_running_double_click_but_not_pending_rerun(tmp_path):
    ...
    # gallery-build params are the (qualities, max_per_cat) pair, so the cap lands in the
    # dedup key — changing only the cap is genuinely different work, not a double-click.
    assert [j["params"] for j in manager.status()["queue"]] == [[("gallery",), None]]   # status() converts only the outer level
