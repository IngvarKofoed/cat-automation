...
- **Browser automation (Playwright MCP)** — for verifying the **config UI** (`server/ui/`) in a real browser: the live/overlay preview and the tuning controls. The tools are deferred `mcp__playwright__*`; load them via `ToolSearch` (keyword `playwright browser`, or `select:mcp__playwright__browser_navigate,mcp__playwright__browser_snapshot,mcp__playwright__browser_console_messages,mcp__playwright__browser_network_requests`). Not needed for capture/motion/backend work.
...
- **Verification workflow (on-device / against fakes).** The camera and GPIO can't be meaningfully unit-tested, so verify edge changes by running the service (on the Pi, or against a fake capture source) and:
  ...
  3. For **config-UI changes**, drive the page in a real browser with the Playwright MCP (`mcp__playwright__*`) tools: confirm it reflects and persists settings (clip, rotation, fps, motion tuning), the Live/overlay preview renders, and check the console + network for errors.
